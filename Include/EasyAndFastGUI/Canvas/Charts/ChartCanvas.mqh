...
class CChartCanvas : public CCustomCanvas
  {
protected:
   //--- Ёкземпл€р класса дл€ работы с цветом
   CColors           m_clr;
   //--- colors
   uint              m_color_background;
   uint              m_color_background2;
   uint              m_color_border;
   uint              m_color_text;
   uint              m_color_grid;
   //--- adjusted parameters
...
public:
   ...
   uint              ColorBackground2(void) const { return(m_color_background2); }
   void              ColorBackground2(const uint value);//
   ...
   void              ScaleDigits(const int value);//
   //--- show flags
   ...
   void              StyleGrid(const ENUM_LINE_STYLE style) { m_style_grid=style; }
...
protected:
   ...
   //---
   void              InitColorArray(const color clr1,const color clr2,color &color_array[]);//
  };
//+------------------------------------------------------------------+
//| Constructor                                                      |
//+------------------------------------------------------------------+
CChartCanvas::CChartCanvas(void) : m_color_background(XRGB(0xFF,0xFF,0xFF)),
                                   m_color_background2(clrNONE),
...
//| Delete all                                                       |//
//+------------------------------------------------------------------+
void CChartCanvas::DeleteAll(void)
  {
   if(m_data!=NULL)
      delete m_data;
//---
...
//--- call method of parent class
   if(!CCustomCanvas::Create(name,width,height,clrfmt))
      return(false);
//--- set font
   FontSet("Calibri",-80);
//--- succeed
...
//| Sets background color                                            |
//+------------------------------------------------------------------+
void CChartCanvas::ColorBackground2(const uint value)
  {
   m_color_background2=value;
//--- redraw
   if(m_data_total>0)
      Redraw();
  }
...
//| Set scale digits                                                 |
//+------------------------------------------------------------------+
void CChartCanvas::ScaleDigits(int value)
  {
   m_scale_digits=value;
  }
//+------------------------------------------------------------------+
...
//---
   color array_color[];
   ArrayResize(array_color,m_height);
   m_color_background2=(m_color_background2==clrNONE) ? m_color_background : m_color_background2;
//---
   InitColorArray(m_color_background,m_color_background2,array_color);
//---
   for(int i=0; i<m_height; i++)
      Line(0,i,m_width,i,ColorToARGB(array_color[i]));
//---
   if(m_color_border!=clrNONE)
      Rectangle(0,0,m_width-1,m_height-1,ColorToARGB(m_color_border));
  }
//+------------------------------------------------------------------+
//| Redraws "legend"                                                 |
//+------------------------------------------------------------------+
...
//--- draw
   switch(m_legend_alignment)
     {
      case ALIGNMENT_LEFT:
         m_data_area.left+=DrawLegendVertical(w,9);
         break;
      case ALIGNMENT_RIGHT:
         m_data_area.right-=DrawLegendVertical(w,9);
         break;
      case ALIGNMENT_TOP:
         m_data_area.top+=DrawLegendHorizontal(w,9);
         break;
      case ALIGNMENT_BOTTOM:
         m_data_area.bottom-=DrawLegendHorizontal(w,9);
         break;
     }
  }
//+------------------------------------------------------------------+
      ...
      for(i=0; i<(int)m_data_total; i++,x+=dx)
        {
         ...
         FillRectangle(x,y,x+h,y+h,(uint)m_colors[i]);
         TextOut(x+text_x_gap,y-text_y_gap,m_descriptors[i],m_color_text);
        }
     }
   else
     {
      for(i=0; i<(int)m_index_size; i++,x+=dx)
        {
         ...
         FillRectangle(x,y,x+h,y+h,(uint)m_colors[index]);
         TextOut(x+text_x_gap,y-text_y_gap,m_descriptors[index],m_color_text);
        }
      if(i%cols==0)
        {
         x=dx-w/2+h;
      ...
      FillRectangle(x,y,x+h,y+h,COLOR2RGB(clrBlack));
      TextOut(x+text_x_gap,y-text_y_gap,"Others",m_color_text);
     }
//--- height
   return(dy*(rows+1));
  }
...
//--- add indent and graduation mark
   size+=20;
//--- draw
   if(draw)
     {
...
//--- draw
   ...
   for(uint i=0; i<=j; i++,y-=m_dy_grid)
      LineAA(x1,y,x2,y,m_color_grid,m_style_grid);
  }
//+------------------------------------------------------------------+
//| Redraws data                                                     |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| »нициализаци€ массива переходом от одного цвета к другому        |//
//+------------------------------------------------------------------+
void CChartCanvas::InitColorArray(const color clr1,const color clr2,color &color_array[])
  {
//--- ћассив цветов градиента
   color colors[2];
   colors[0]=clr1;
   colors[1]=clr2;
//--- ‘ормирование массива цветов
   m_clr.Gradient(colors,color_array,m_height);
  }
//+------------------------------------------------------------------+
